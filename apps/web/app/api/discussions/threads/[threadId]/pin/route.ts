...
import { submitThreadPinToggle } from "../../../../../../src/discussion/discussion-actions";

interface ThreadPinRouteProps {
  params: Promise<{
    threadId: string;
  }>;
}

export async function POST(request: Request, props: ThreadPinRouteProps): Promise<NextResponse> {
  ...
  try {
    const result = await submitThreadPinToggle({
      actor,
      market,
      pinned: readRequiredString(formData.get("pinned")) === "1",
      threadId: params.threadId,
      ticker
    });

    return NextResponse.redirect(withFeedbackUrl(request.url, result.canonicalPath, actor, "thread_pinned", undefined, params.threadId), 303);
  } catch (error) {
