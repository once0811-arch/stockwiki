...
import { submitHelpfulVote } from "../../../../../../src/discussion/discussion-actions";

interface HelpfulVoteRouteProps {
  params: Promise<{
    commentId: string;
  }>;
}

export async function POST(request: Request, props: HelpfulVoteRouteProps): Promise<NextResponse> {
  ...
  try {
    const result = await submitHelpfulVote({
      actor,
    ...
    return NextResponse.redirect(withFeedbackUrl(request.url, result.canonicalPath, actor, "helpful_updated", undefined, result.commentId), 303);
  } catch (error) {
